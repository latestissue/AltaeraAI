...
while IFS= read -r -d $'\0' file; do
    files+=("$file" "")
done < <(find "/data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/altaera/root/koboldcpp-altaera" -maxdepth 1 -type f -name "*.bin" -print0) PROMPT_DIRTRIM=3
